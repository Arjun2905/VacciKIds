...
class MyVaccineScheduleScreen extends State<VaccineScheduleScreen> {
  ...
  Future<String> getDate(String childId) async {
          ...
          if (!snapshot.hasData) {
            ...
            return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if(!documentSnapshot['duration'].toString().contains("range"))
                  {
                    NotificationService().scheduleNotification(
                        getDate(widget.childId).toString(),
                        documentSnapshot['duration']);
                    print("notification set");
                  }else{
                    String duration = documentSnapshot['duration'].toString();
                    var list = duration.split(",");
                    duration = list[0].substring(7);
                    NotificationService().scheduleNotification(
                        getDate(widget.childId).toString(),
                        duration);
                    print("notification set");
